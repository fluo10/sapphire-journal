...
/// per-journal `.sapphire-journal/config.toml` so that the same journal can be shared
...
impl UserConfig {
    ...
    /// (or `~/.config/sapphire-journal/config.toml` when `XDG_CONFIG_HOME` is not set).
    pub fn path() -> PathBuf {
    ...
    /// | Variable                                | Field                              |
    /// |-----------------------------------------|------------------------------------|
    /// | `SAPPHIRE_JOURNAL_CACHE_EMBEDDING_ENABLED`      | `cache.embedding.enabled`          |
    /// | `SAPPHIRE_JOURNAL_CACHE_EMBEDDING_VECTOR_DB`    | `cache.embedding.vector_db`        |
    /// | `SAPPHIRE_JOURNAL_CACHE_EMBEDDING_PROVIDER`     | `cache.embedding.provider`         |
    /// | `SAPPHIRE_JOURNAL_CACHE_EMBEDDING_MODEL`        | `cache.embedding.model`            |
    /// | `SAPPHIRE_JOURNAL_CACHE_EMBEDDING_API_KEY_ENV`  | `cache.embedding.api_key_env`      |
    /// | `SAPPHIRE_JOURNAL_CACHE_EMBEDDING_BASE_URL`     | `cache.embedding.base_url`         |
    /// | `SAPPHIRE_JOURNAL_CACHE_EMBEDDING_DIMENSION`    | `cache.embedding.dimension`        |
    ...
    fn apply_env_overrides(&mut self) {
        ...
        let vector_db = std::env::var("SAPPHIRE_JOURNAL_CACHE_EMBEDDING_VECTOR_DB").ok()
