...
/**
 * Walk up the directory tree from `filePath` looking for a `.sapphire-journal/` directory.
 * Returns the journal root path, or null if not found.
 */
export function findJournalRoot(filePath: string): string | null {
    let dir = path.dirname(filePath);
    while (true) {
        if (fs.existsSync(path.join(dir, '.sapphire-journal'))) {
            return dir;
        }
...
/**
 * Returns true if the filename looks like an sapphire-journal-managed entry:
