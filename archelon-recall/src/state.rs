...
use anyhow::Result;
use archelon_retrieve::{Embedder, RetrieveDb, db::SCHEMA_VERSION};
...
impl WorkspaceState {
    ...
    pub fn load_embedder(&self, config: &UserConfig) -> Result<()> {
        ...
        let embedder = config
            ...
            .filter(|c| c.enabled)
            .map(|c| {
                archelon_retrieve::build_embedder(&c.to_retrieve_embed_config())
    ...
    pub fn db_info(&self) -> Result<DbInfo> {
        ...
        let vec_info = self.retrieve_db.vec_info().unwrap_or(archelon_retrieve::VecInfo {
            embedding_dim: 0,
            vector_count: 0,
            pending_count: 0,
        });
