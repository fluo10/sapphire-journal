...
impl Workspace {
    ...
    /// 2. `SAPPHIRE_RETRIEVE_WORKSPACE` env var (no confirmation prompt)
    ...
    pub fn resolve(explicit: Option<&Path>) -> Result<Self> {
        ...
        } else if let Ok(val) = std::env::var("SAPPHIRE_RETRIEVE_WORKSPACE") {
            if !val.is_empty() {
                Path::new(&val)
                    .canonicalize()
                    .map_err(|e| anyhow!("cannot access SAPPHIRE_RETRIEVE_WORKSPACE={val}: {e}"))?
            } else {
                resolve_cwd()?
            }
        } else {
    ...
    /// `$XDG_CACHE_HOME/sapphire-retrieve-cli/{hash16}-{basename}/`
